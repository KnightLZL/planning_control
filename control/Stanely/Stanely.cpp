 ...
 * @return
 */
double Stanely::calTargetIndex(vector<double> robot_state, vector<vector<double>> refer_path) {
    vector<double>dists;
 ...
 * @return
 */
double Stanely::normalizeAngle(double angle) {
    while(angle>PI){
        angle-=2.0*PI;
    }
    while(angle<-PI){
...
/**
 * stanley控制
 * @param robot_state 车辆位姿，包括x,y,yaw,v
 * @param refer_path 参考轨迹的位置和参考轨迹上点的切线方向的角度 x,y,theta
 * @return 控制量+目标点索引
 */

vector<double> Stanely::stanelyControl(vector<double> robot_state, 
vector<vector<double>> refer_path) {
    double current_target_index = Stanely::calTargetIndex(robot_state, refer_path);

    vector<double> current_ref_point;

    // 当计算出的目标临近索引点，且大于等于参考轨迹上的最后一个点索引时
    if(current_target_index >= refer_path.size()-1) {
