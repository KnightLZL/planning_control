...
#ifndef PLANNING_CONTROL_AUTO_DUBINS_H
#define PLANNING_CONTROL_AUTO_DUBINS_H
...
class Dubins{
    ...
    struct Path{
        double t=-0., p=0., q=0; //设定一个不会出现的初始值
        string mode;
    };
    ...
    Path right_left_right(double alpha, double beta, double d);//RLR
    ...
    vector<vector<double>> interpolate(double ind, double length, char m, double max_curvature, double origin_x, double origin_y,
                                    double origin_yaw, vector<double> path_x,
                                    vector<double> path_y, vector<double> path_yaw, vector<double>directions);

    vector<vector<double>> generate_local_course(double total_length,
        vector<double>lengths, string modes, double max_curvature, double step_size);
