...
vector<double> Dubins::polar(double x, double y){
    ...
    double theta = atan2(y, x);  //范围在[-pi,pi]
    return {r, theta};
}

/**
 * theta对2pi取模，限制到【0，2pi】符合极坐标系，周期范围
*/
...
Dubins::Path Dubins::left_straight_left(double alpha, double beta, double d){
    ...
    path.t = mod2Pi(-alpha + tmp1);  //起点弧长对应的角
    path.p = sqrt(p_squared); //直线行驶的长
    path.q = mod2Pi(beta - tmp1); //终点弧长对应的角
 ...
 * 生成平滑的局部路径
*/
vector<vector<double>>Dubins::generate_local_course(double total_length, vector<double> lengths, string modes, double max_curvature, double step_size){
    double n_point = trunc(total_length / step_size) + lengths.size() +4;// trunc取整数部分
    vector<double>p_x(n_point, 0.0), p_y(n_point, 0), p_yaw(n_point, 0),directions(n_point, 0.0);
    double ind = 1;
    if(lengths[0] > 0.0){
        directions[0] = 1;
    }else{
    ...
    for(int i=0; i<modes.size(); i++){
        ...
        {
            ind += 1;
            vector<vector<double>> inter = interpolate(ind , pd, modes[i],   max_curvature, origin_x, origin_y,origin_yaw, p_x, p_y, p_yaw, directions);
            p_x = inter[0], p_y = inter[1], p_yaw = inter[2], directions = inter[3];
            pd += dist;


        }
        ll = lengths[i] - pd - dist;// 计算剩余长度
        ind += 1;
        vector<vector<double>>inter = interpolate(ind, lengths[i], modes[i],
                                                  max_curvature, origin_x, origin_y, origin_yaw,
                                                  p_x, p_y,p_yaw,
                                                  directions);
        p_x = inter[0], p_y = inter[1], p_yaw = inter[2], directions = inter[3];
...
Dubins::ResultDubins Dubins::dubins_path_planning_from_origin(Vector3d goal,
